...
function AppointmentModal({ isOpen, onClose, date, timeSlot, appointment }) {
    ...
    const createMutation = useMutation({
        ...
        onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['appointments'] }); notify.success(t('pages.archives.appointmentCreated')); onClose(); }
    });
    const updateMutation = useMutation({
        mutationFn: (data) => appointmentsApi.update(appointment.id, data),
        onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['appointments'] }); notify.success(t('pages.archives.appointmentUpdated')); onClose(); }
    });
    const deleteMutation = useMutation({
        mutationFn: () => appointmentsApi.delete(appointment.id),
        onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['appointments'] }); notify.success(t('pages.archives.appointmentDeleted')); onClose(); }
    });
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div className="relative bg-card border border-border rounded-xl shadow-xl w-full max-w-md p-6 select-text">
                ...
                <div className="space-y-4">
                    ...
                    <textarea value={message} onChange={(e) => setMessage(e.target.value)} placeholder={t('components.bulkMessage.message')}
                    ...
                    <div className="flex gap-3 pt-2">
                        <button onClick={onClose} className="flex-1 px-4 py-2 border border-border rounded-lg hover:bg-muted">{t('pages.archives.cancel')}</button>
                        <button onClick={() => {
...
export default function Archives() {
    ...
    const advanceWorkflowMutation = useMutation({
        ...
        onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['archives'] }); notify.success(t('pages.archives.statusUpdated')); setWorkflowPopup(null); },
                ...
                <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center gap-2">
                        <button onClick={() => setShowFilters(f => !f)}
                            ...
                            {hasActiveFilters ? `${t('pages.archives.filters')} (${[filterShootType, filterLocation, filterSchool, filterStatus].filter(Boolean).length})` : t('pages.archives.filters')}
                        </button>
