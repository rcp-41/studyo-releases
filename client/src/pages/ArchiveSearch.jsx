...
export default function ArchiveSearch() {
    ...
    const [filters, setFilters] = useState({
        ...
        dateFrom: '', dateTo: '',
        shootTypeId: '', locationId: '', workflowStatus: '', paymentStatus: '',
        photographerId: '', schoolId: '', className: ''
    });
    ...
    // Fetch archives — pass filters to server when applied
    const { data: archivesData, isLoading } = useQuery({
        queryKey: ['archives', 'search', appliedFilters],
        queryFn: () => {
            // Build server-side filters from applied filters
            const serverFilters = {};
            if (appliedFilters.dateFrom) serverFilters.dateFrom = appliedFilters.dateFrom;
            if (appliedFilters.dateTo) serverFilters.dateTo = appliedFilters.dateTo;
            if (appliedFilters.shootTypeId) serverFilters.shootTypeId = appliedFilters.shootTypeId;
            if (appliedFilters.locationId) serverFilters.locationId = appliedFilters.locationId;
            if (appliedFilters.photographerId) serverFilters.photographerId = appliedFilters.photographerId;
            if (appliedFilters.schoolId) serverFilters.schoolId = appliedFilters.schoolId;
            if (appliedFilters.className) serverFilters.className = appliedFilters.className;
            if (appliedFilters.workflowStatus) serverFilters.workflowStatus = appliedFilters.workflowStatus;

            const hasFilters = Object.keys(serverFilters).length > 0;
            return archivesApi.list({
                ...(hasFilters ? { filters: serverFilters } : {}),
                limit: hasFilters ? 5000 : 1000
            });
        }
    });
    ...
    const handleClear = () => {
        const empty = { name: '', phone: '', archiveNo: '', dateFrom: '', dateTo: '', shootTypeId: '', locationId: '', workflowStatus: '', paymentStatus: '', photographerId: '', schoolId: '', className: '' };
        setFilters(empty);
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3">
                    ...
                    <div>
                        ...
                        {(() => {
                            ...
                            return <input type="text" value={filters.className} onChange={e => setFilters({ ...filters, className: e.target.value })}
                                placeholder="Ör: 4-A" className="w-full px-3 py-2 rounded-lg bg-background border border-input focus:border-primary outline-none text-sm" />;
                        })()}
                    </div>
                    <div>
