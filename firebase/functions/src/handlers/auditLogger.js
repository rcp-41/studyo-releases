 ...
 * Multi-Tenant: organizations/{orgId}/studios/{studioId}/auditLogs
 */
    ...
    try {
        ...
        if (organizationId && studioId) {
            await db.collection('organizations').doc(organizationId)
                ...
                .collection('auditLogs').add(logEntry);
        } else {
...
exports.getLogs = onCall({ enforceAppCheck: false }, async (request) => {
    ...
    if (!studioId || !organizationId) {
        throw new HttpsError('failed-precondition', 'studioId/organizationId not found');
    }
 ...
 */
async function logActivity(organizationId, studioId, action, userId, details = {}, targetId = null) {
    try {
        const logEntry = {
            action,
            userId,
