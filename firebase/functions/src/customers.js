    ...
    try {
        ...
        const shoots = snapshot.docs.map(doc => {
            ...
            return {
                id: doc.id,
                ...data,
                // Normalize date fields for frontend
                shootDate: data.createdAt?.toDate?.() ? data.createdAt.toDate().toISOString() : data.createdAt,
                createdAt: data.createdAt?.toDate?.() ? data.createdAt.toDate().toISOString() : data.createdAt,
                shootCode: data.archiveId || data.archiveNumber || doc.id
            };
        });

        // Sort by date descending
        shoots.sort((a, b) => new Date(b.shootDate || 0) - new Date(a.shootDate || 0));
