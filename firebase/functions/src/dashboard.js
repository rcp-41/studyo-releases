...
exports.getFilteredStats = onCall({ memory: '256MiB' }, async (request) => {
    ...
    try {
        ...
        // Online satışlar (ayrı koleksiyon)
        let onlineCount = 0;
        try {
        ...
        } catch (e) { console.warn('Query skipped:', e?.message); }
...
exports.getMonthlyStats = onCall({ enforceAppCheck: false }, async (request) => {
    ...
    try {
        ...
        // Günlük gelir dökümü — chart için {date, amount} dizisi
        const dailyMap = {};
...
exports.getCalendarView = onCall({ enforceAppCheck: false }, async (request) => {
    ...
    try {
        // Parse date strings and create UTC-consistent date range
        // IMPORTANT: new Date('yyyy-MM-dd') creates UTC midnight, but setHours() uses local time
        // This caused a timezone mismatch — we now explicitly use UTC to avoid this
        ...
        // Try Timestamp-based query first, then fallback to string-based
        const appointmentsRef = dbHandler.collection('appointments');
        ...
        try {
            ...
            // Fallback: string-based query for legacy data
            appointmentsSnapshot = await appointmentsRef
                .where('appointmentDate', '>=', start.toISOString())
                .where('appointmentDate', '<=', end.toISOString())
                .orderBy('appointmentDate', 'asc')
                .get();
        }

        // If Timestamp query found nothing, try loading all and filtering manually
        // This handles mixed-type data (some Timestamps, some strings)
        if (appointmentsSnapshot.empty) {
            try {
                const allSnap = await appointmentsRef.orderBy('createdAt', 'desc').limit(2000).get();
                const filtered = [];
                allSnap.docs.forEach(doc => {
                    const data = doc.data();
                    let aptDate = data.appointmentDate?.toDate?.()
                        || (typeof data.appointmentDate === 'string' ? new Date(data.appointmentDate) : null)
                        || (data.appointmentDate instanceof Date ? data.appointmentDate : null);
                    if (aptDate && aptDate >= start && aptDate <= end) {
                        filtered.push(doc);
                    }
                });
                if (filtered.length > 0) {
                    appointmentsSnapshot = { docs: filtered, empty: false };
                }
            } catch (fallbackErr) {
                console.error('Calendar fallback query failed:', fallbackErr);
            }
        }
        ...
        let shootsSnapshot;
        try {
            shootsSnapshot = await shootsRef
                .where('shootDate', '>=', startDate)
                .where('shootDate', '<=', endDate)
                .orderBy('shootDate', 'asc')
                .get();
        } catch (e) {
            console.warn('Query skipped:', e?.message);
            shootsSnapshot = { docs: [], empty: true };
        }
        ...
        const appointments = (appointmentsSnapshot.docs || []).map(doc => {
            const data = doc.data();
            const aptDate = resolveDate(data.appointmentDate) || resolveDate(data.createdAt) || new Date();
            return {
                id: doc.id,
                type: 'appointment',
                ...data,
                // Serialize all dates to ISO strings for frontend compatibility
                appointmentDate: aptDate.toISOString(),
                createdAt: resolveDate(data.createdAt)?.toISOString() || data.createdAt,
                updatedAt: resolveDate(data.updatedAt)?.toISOString() || data.updatedAt,
                importedAt: resolveDate(data.importedAt)?.toISOString() || data.importedAt
            };
        });
