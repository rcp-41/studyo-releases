...
// G5: generateTotpSecret alias kept for backwards compat — enable2FA IS the generator
// disableTotp callable added below
...
// G5: Disable TOTP for a creator user
exports.disableTotp = onCall({ enforceAppCheck: false }, async (request) => {
    if (!request.auth) {
        throw new HttpsError('unauthenticated', 'Must be logged in');
    }
    if (!TOTP_ENCRYPTION_KEY) {
        throw new HttpsError('failed-precondition', '2FA yapılandırması eksik.');
    }

    const { code } = request.data || {};
    if (!code || code.length !== 6) {
        throw new HttpsError('invalid-argument', 'Devre dışı bırakmak için 6 haneli TOTP kodu gerekli');
    }

    try {
        const userDoc = await db.collection('users').doc(request.auth.uid).get();
        if (!userDoc.exists || !userDoc.data().totpSecret || !userDoc.data().totpEnabled) {
            throw new HttpsError('failed-precondition', '2FA zaten kapalı');
        }
        ...
        const currentUser = await auth.getUser(request.auth.uid);
        const currentClaims = currentUser.customClaims || {};
        delete currentClaims.totp_enabled;
        await auth.setCustomUserClaims(request.auth.uid, currentClaims);

        return { success: true };
    } catch (error) {
        if (error instanceof HttpsError) throw error;
        console.error('disableTotp error:', error);
        throw new HttpsError('internal', '2FA devre dışı bırakma başarısız');
    }
});
