...
exports.assignPhotographer = onCall({ enforceAppCheck: false }, async (request) => {
    ...
    try {
        ...
        // Read photographer from studio-scoped users subcollection
        const photographerDoc = await db.collection('studios').doc(studioId).collection('users').doc(photographerId).get();
