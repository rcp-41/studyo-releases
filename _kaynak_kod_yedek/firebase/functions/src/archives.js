...
exports.list = onCall(async (request) => {
    ...
    const { search, limit: queryLimit, filters } = request.data || {};
    ...
    try {
        ...
        if (searchTrimmed) {
            ...
            // Advanced filters: locationId, schoolId, className
            const snapshot = await dbHandler.collection('archives').orderBy('createdAt', 'desc').limit(500).get();
            archives = snapshot.docs.map(mapDoc).filter(a => {
                if (filters.locationId && a.locationId !== filters.locationId) return false;
                if (filters.schoolId && a.schoolId !== filters.schoolId) return false;
                if (filters.className && a.className !== filters.className) return false;
                if (filters.shootTypeId && a.shootTypeId !== filters.shootTypeId) return false;
                return true;
            });
        } else {
            const maxLimit = Math.min(queryLimit || 100, 500);
            const snapshot = await dbHandler.collection('archives').orderBy('createdAt', 'desc').limit(maxLimit).get();
            archives = snapshot.docs.map(mapDoc);
        }

        return { data: archives, total: archives.length };
    } catch (error) {
...
exports.transferFromAppointment = onCall({ enforceAppCheck: false }, async (request) => {
    ...
    try {
        const result = await dbHandler.runTransaction(async (transaction) => {
            ...
            const aptRef = dbHandler.doc('appointments', appointmentId);
            ...
            const counterRef = dbHandler.doc('counters', 'archive');
