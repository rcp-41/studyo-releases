 ...
 * 
 * Multi-Tenant: Users are stored under studios/{studioId}/users/{uid}
 */
...
// Helper: get studio-scoped users collection reference
const usersCollection = (studioId) => db.collection('studios').doc(studioId).collection('users');
...
exports.getAll = onCall({ enforceAppCheck: false }, async (request) => {
    const studioId = requireAdmin(request);

    try {
        // Studio-scoped users subcollection — artık filtreye gerek yok
        const snapshot = await usersCollection(studioId)
...
exports.create = onCall({ enforceAppCheck: false }, async (request) => {
    ...
    // Build auth email: use provided email or generate from username
    ...
    try {
        ...
        await usersCollection(studioId).doc(userRecord.uid).set({
            email: authEmail,
            username: username || '',
            fullName,
            phone: phone || '',
...
exports.update = onCall({ enforceAppCheck: false }, async (request) => {
    ...
    const userDoc = await usersCollection(studioId).doc(uid).get();
    if (!userDoc.exists) {
    ...
    try {
        ...
        await usersCollection(studioId).doc(uid).update(firestoreUpdates);
...
exports.delete = onCall({ enforceAppCheck: false }, async (request) => {
    ...
    // Verify user belongs to this studio
    const userDoc = await usersCollection(studioId).doc(id).get();
    if (!userDoc.exists) {
        throw new HttpsError('not-found', 'Kullanıcı bulunamadı.');
    }
    if (userDoc.data().studioId !== studioId) {
    ...
    try {
        ...
        await usersCollection(studioId).doc(id).delete();
        return { success: true };
    } catch (error) {
 ...
 */
exports.toggleStatus = onCall({ enforceAppCheck: false }, async (request) => {
    const studioId = requireAdmin(request);
    ...
    try {
        ...
        await usersCollection(studioId).doc(id).update({
            isActive: newActive,
 ...
 * Get staff leaves
 */
exports.getLeaves = onCall({ enforceAppCheck: false }, async (request) => {
    const studioId = requireAdmin(request);

    try {
        // SECURITY: Studio-scoped leaves subcollection
        const snapshot = await db.collection('studios').doc(studioId).collection('leaves')
 ...
 * Add staff leave
 */
exports.addLeave = onCall({ enforceAppCheck: false }, async (request) => {
    const studioId = requireAdmin(request);
    ...
    // Verify user belongs to this studio
    const userDoc = await usersCollection(studioId).doc(userId).get();
    ...
    try {
        ...
        // SECURITY: Studio-scoped leaves subcollection
        const docRef = await db.collection('studios').doc(studioId).collection('leaves').add(leaveData);
 ...
 * Delete staff leave
 */
exports.deleteLeave = onCall({ enforceAppCheck: false }, async (request) => {
    const studioId = requireAdmin(request);
    ...
    try {
        // SECURITY: Studio-scoped leaves subcollection
        const leaveDoc = await db.collection('studios').doc(studioId).collection('leaves').doc(id).get();
        if (!leaveDoc.exists) {
