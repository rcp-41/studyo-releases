...
export const settingsApi = {
    ...
    update: async (section, data) => callFunction('settings-update', { key: section, value: data }),
...
// WhatsApp Service (Electron IPC → Baileys)
export const whatsappApi = {
    ...
    send: ({ phone, message }) => {
        if (window.electron?.whatsapp?.send) {
            return window.electron.whatsapp.send(phone, message);
        }
        return callFunction('whatsapp-send', { phone, message });
    },
