...
import {
    ...
    MessageCircle, Globe, HardDrive, Folder, SlidersHorizontal, UserCircle, Archive
} from 'lucide-react';
...
function ArchiveModal({ isOpen, onClose, archive }) {
    ...
    const createMutation = useMutation({
        ...
        onSuccess: async (response) => {
            ...
            // Create folder if base path is configured
            if (basePath && window.electron?.createFolder) {
                try {
                    const folderPath = `${basePath}\\${archive.id}`;
                        ...
                        {schools && schools.length > 0 && (
                            <div className="grid grid-cols-3 gap-3 mt-3">
                                ...
                                <div>
                                    ...
                                    <input type="text" value={formData.section} onChange={e => setFormData({ ...formData, section: e.target.value })} placeholder="Ör: A, B, C..." className="w-full px-3 py-2 rounded-lg bg-background border border-input focus:border-primary outline-none text-sm" />
                                </div>
                            </div>
                        )}
                    </div>
...
export default function Archives() {
    ...
    const archives_raw = archivesData?.data || [];
    const basePath = settings?.general?.archive_base_path;
    ...
    return (
        <div className="flex gap-6 h-[calc(100vh-8rem)] select-none">
            {/* Archive List - 2/3 width */}
            <div className="flex-1 flex flex-col min-w-0">
                <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center gap-2">
                        <button onClick={() => setShowFilters(!showFilters)}
                            className={`flex items-center gap-2 px-4 py-2 border rounded-lg text-sm ${hasActiveFilters ? 'border-primary bg-primary/10 text-primary' : 'border-border hover:bg-muted'
                                }`}>
                            <SlidersHorizontal className="w-4 h-4" />
                            Filtrele {hasActiveFilters && <span className="w-2 h-2 rounded-full bg-primary" />}
                        </button>
                ...
                {/* Filter Bar */}
                {showFilters && (
                    <div className="flex items-center gap-3 mb-3 p-3 bg-muted/30 border border-border rounded-xl animate-in slide-in-from-top-2">
                        <select value={filterShootType} onChange={e => setFilterShootType(e.target.value)}
                            className="px-3 py-1.5 rounded-lg bg-background border border-input text-sm focus:border-primary outline-none min-w-[140px]">
                            <option value="">Tüm Çekim Türleri</option>
                            {shootTypes?.map(st => <option key={st.id} value={st.id}>{st.name}</option>)}
                        </select>
                        <select value={filterLocation} onChange={e => setFilterLocation(e.target.value)}
                            className="px-3 py-1.5 rounded-lg bg-background border border-input text-sm focus:border-primary outline-none min-w-[140px]">
                            <option value="">Tüm Çekim Yerleri</option>
                        ...
                        <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)}
                            ...
                            {Object.entries(WORKFLOW_STATUSES).map(([key, ws]) => <option key={key} value={key}>{ws.label}</option>)}
                        </select>
                        {hasActiveFilters && (
                            <button onClick={clearAllFilters} className="text-xs text-red-500 hover:text-red-600 whitespace-nowrap flex items-center gap-1">
                                <X className="w-3 h-3" /> Temizle
                            </button>
                        )}
                        <span className="text-xs text-muted-foreground ml-auto">{archives.length} kayıt</span>
                    </div>
                )}
                        ...
                        <table className="w-full text-sm">
                            ...
                            <tbody className="divide-y divide-border">
                                {isLoading ? (
                                    ...
                                    <tr><td colSpan={7} className="text-center py-8 text-muted-foreground">Kayıt bulunamadı</td></tr>
                                ) : (
                                    archives.map(arc => (
                                        <tr
                                            key={arc.id}
                                            ...
                                            <td className="px-3 py-2">
                                                <div className="flex items-center justify-end gap-1">
                                                    ...
                                                    <button
                                                        ...
                                                        onClick={async (e) => {
                                                            e.stopPropagation();
                                                            const folderPath = arc.folderPath || (basePath ? `${basePath}\\${arc.id}` : null);
