...
const FIREBASE_HOSTING_URL = process.env.FIREBASE_HOSTING_URL || 'https://studyo-manager.web.app';
...
function createWindow() {
    ...
    if (isDev) {
        ...
        // Load from local dist (Studio Mode / Offline capable)
...
app.whenReady().then(() => {
    ...
    try {
        ...
        if (fs.existsSync(configPath)) {
            const licenseConfig = JSON.parse(fs.readFileSync(configPath, 'utf8'));
...
function migrateLicenseConfig() {
    try {
        ...
        if (config.archiveBasePath && !config.studios) {
            console.log('[Migration] Migrating archiveBasePath to studios array...');
            config.studios = [{
                id: 'default',
                name: 'Varsayılan Stüdyo',
                path: config.archiveBasePath
            }];
            // We keep archiveBasePath for now just in case, or we could remove it.
            // Requested behavior: "change the structure".
            // Let's remove it to ensure we rely on the new structure, but since existing code might check it?
            // User script: "Yani yapıyı değiştirsin... studios diye bir array'e çevirsin."
        ...
        if (changed) {
            ...
            console.log('[Migration] License config migration completed.');
        }
    } catch (error) {
